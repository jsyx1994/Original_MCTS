...
inline void MctsNode::updateEntropy() {
    double Px;
    // 计算反向路径上节点的熵
    ChildZone children = this->getChildren();
    ...
    for(int i = 0; i < len; ++i)
    {
        Px = children[i]->getVisit() / (this->getVisit()-1);
        //        cout << Px << endl;
        double delta = -Px * log2(Px);
        if (isnan(delta))
            delta = 0;
        Hp +=  delta;
    }
...
inline double MctsNode::getEntropy() {
    return this->entropy;
}

inline void MctsNode::setEntropy(double entropy) {
...
public:
    ...
    inline int judge(MctsNode* node);
private:
    BoardType BoardCopy;
    MctsNode *rootState;
};
...
inline void Bot::simulate(MctsNode* workingNode)
{
    ...
    int winner;
    winner = judge(simuNode);
    backpropagate(workingNode, winner, 1);
}
...
inline MctsNode* Bot::chooseChild(MctsNode* node)
{
    ...
    double MAX = bestchild->getWeight();
    for (auto &r:childzone)
    {
...
inline int Bot::judge(MctsNode* node)
{
...
inline void Bot::RecoverHistory()
{
    ...
    for (int i = 0; i < turnID; i++)
    {
        ...
        if (x >= 0)
            ProcStep(rootState, x, y, -currBotColor); // 模拟对方落子
        
        //outPutState(rootState);
        x = input["responses"][i]["x"].asInt();
        y = input["responses"][i]["y"].asInt();
        if (x >= 0)
            ProcStep(rootState, x, y, currBotColor); // 模拟己方落子
        //outPutState(rootState);
    }
